...
export function StarButton({
  ...
  const innerContent = (
    <>
      ...
      {/* Star background */}
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] border-white/15"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="z-10 relative bg-gradient-to-t from-white to-neutral-300 inline-block text-transparent bg-clip-text font-semibold">
        {children}
      </span>
    </>
  );
